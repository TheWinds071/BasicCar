    ...
    void updateISR(uint8_t conformedQuestion) {
        ...
        switch (conformedQuestion) {
            ...
            case 3: {
                ...
                bool falling = false;
                constexpr uint8_t NO_LINE_N = 18; // 20ms周期下约140ms丢线才认为真正到点
                ...
                switch (_q3_state) {
                    ...
                    case Q3State::Arc_CB:
                        ...
                        if (falling) {
                            ...
                            float target = _q3_yaw_AC_deg + 102.0f;
                            while (target > 180.0f) target -= 360.0f;
                            while (target < -180.0f) target += 360.0f;
            ...
            case 4: {
                ...
                bool falling = false;
                constexpr uint8_t NO_LINE_N = 18;
                ...
                switch (_q4_state) {
                    ...
                    case Q4State::Arc_CB:
                        ...
                        if (falling) {
                            ...
                            Prompt::once(120);
                            //resetYawRef();
                            float target = _q4_yaw_AC_deg + 102.0f;
                            while (target > 180.0f) target -= 360.0f;
                            while (target < -180.0f) target += 360.0f;
                            setYawRefDeg(target);              // ← 用目标角锁航向
